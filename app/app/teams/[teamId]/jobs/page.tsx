...
export default async function TeamJobsPage({
  ...
  const [jobs, allStats, last14Jobs] = await Promise.all([
    ...
    prisma.job.findMany({
      where: { teamId, createdAt: { gte: fourteenDaysAgo } },
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8 space-y-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Team Jobs</h1>
        <p className="text-sm text-muted-foreground mt-1">PDF render jobs for this team.</p>
      </div>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex gap-2 flex-wrap">
          {([undefined, "queued", "processing", "succeeded", "failed"] as const).map((s) => (
            <a
              key={s ?? "all"}
              href={s ? `${base}/jobs?status=${s}` : `${base}/jobs`}
